...
if(isset($_GET['getall'])){
    $zip = new ZipArchive;
    $tmp_file = dirname(__file__) .'/../vqmod_xmls_'.date('YmdHis').'.zip';
    if ($zip->open($tmp_file, ZipArchive::CREATE) === TRUE) {
        foreach (glob(PATH . '*.xml') as $path) {
            $name = basename($path);
            if($name !='vqmod_opencart.xml')
            {
                $zip->addFile($path, 'vqmod/xml/' . $name);
            }
        }
        $zip->setArchiveComment("generaeted at ". date(DATE_ISO8601));
        $zip->close();
        header("Location:./".basename($tmp_file));
    } else {
        echo 'failed';
    }
	exit;
